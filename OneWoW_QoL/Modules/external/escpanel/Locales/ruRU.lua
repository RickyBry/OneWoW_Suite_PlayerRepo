...
OneWoW.Locale:Register(M._scope, "ruRU", {
    ...
    ["ESCPANEL_DESC"] = "Показывает карточку персонажа, коллекции и заметки этого места и полосу порталов рядом с меню ESC. На карточке персонажа - письмо, прочность, Великое хранилище и Торговая лавка, а Предприятия можно включить отдельно. На карточке места значки Предметы показывают Shopping List, заметки, Trackers и фарм. Срабатывания слева от |, остальные справа. Наведите на значок, чтобы увидеть подробности; нажмите срабатывание, чтобы открыть его. Нажмите её, чтобы открыть окно персонажа, или карточку места, чтобы открыть эту зону в Каталоге. Выберите ниже, какую сторону использует каждый элемент.",
