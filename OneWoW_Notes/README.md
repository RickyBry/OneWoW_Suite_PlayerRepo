...
- Pin Packs group many pins behind one row. Turn a pack off to hide every pin in it on the maps. Import or export a pack string. An imported pack works like one you made (add, delete, rename, reorder). Send to Pack moves a pin into a pack (it uses the pack look until you give that pin its own). Right-click a pin or pack for more. Pack pin rows keep Edit only; a line above that list says to right-click for the rest. With a pack selected, Add Here and Find Location add into that pack. Pack Look uses the same icon, background, and effect options as a pin. Expansion lists every current expansion without scrolling. Drag pack pins to reorder them. Show Disabled Packs stays on so you can turn a pack back on.
...
- Open a pin for Go and Show Map (opens that pin's zone and sets a live waypoint). The world map button uses the OneWoW icon. Add Pin, then click the map (ghost pin and live coordinates). Add Here saves at your feet. Find Location is on that button too. Map Click Menu defaults to on with Ctrl-Right so a plain right-click stays with other addons
