...
local function JournalTypeOptions(includeFarming)
    local opts = {
        { text = L["NOTE_TYPE_STANDARD"], value = "standard" },
        { text = DAILY, value = "daily" },
        { text = WEEKLY, value = "weekly" },
    }
    if includeFarming then
        opts[#opts + 1] = { text = L["NOTE_TYPE_FARMING"], value = "farming" }
    end
    return opts
end

function ns.UI.ShowAddNoteDialog(presetType)
    if presetType == "farming" or presetType == "item" then
        presetType = "standard"
    end
        ...
        buttons = {
            {
                ...
                onClick = function(dlg)
                    ...
                    if selectedNoteType == "farming" or selectedNoteType == "item" then
    ...
    noteTypeDD:SetOptions(JournalTypeOptions(false))
...
function ns.UI.ShowNotePropertiesDialog(noteID)
    ...
    local noteTypeDD = ns.UI.CreateThemedDropdown(content, "", COL_W, 26)
    noteTypeDD:SetPoint("TOPLEFT", content, "TOPLEFT", COL1_X, yPos - LBL_GAP)
    noteTypeDD:SetOptions(JournalTypeOptions(noteData.noteType == "farming"))
